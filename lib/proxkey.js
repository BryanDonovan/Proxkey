...
function setHandlerForRoute(args, cb) {
	...
	var handler = function (request, reply) {
		...
		if (!_.isEmpty(query_params)) {
			...
			//search for query_param in request query
			if (Proxkey.Responses[request.route.path].params) {
				...
				response_mode.forEach(function(val1) {
					var tmp_current_mode = val1;
					if (Proxkey.Responses[request.route.path].params[val1] && Proxkey.Responses[request.route.path].params[val1].length > 0){
						Proxkey.Responses[request.route.path].params[val1].forEach(function(val2,key2) {
							var tmp_cmp_obj = {};
							tmp_cmp_obj[val2.key] = val2.value;

							if (_.isEqual(tmp_cmp_obj,query_params)) {

								if (tmp_current_mode == 'production') { //Proxy Mode
									console.log(tmp_current_mode);
									console.log('return pxxxx');
									if (!Proxkey.Responses[request.route.path].response[tmp_current_mode]) {
										throw new Error(Errors.RouteProductionMustHaveURLToProxy);
									}

									if (!Proxkey.Responses[request.route.path].response[tmp_current_mode].host || !Proxkey.Responses[request.route.path].response[tmp_current_mode].path || !Proxkey.Responses[request.route.path].response[tmp_current_mode].port) {
										throw new Error(Errors.RouteProductionMustHaveHostPathPort);
									}
									//Go and hit Production URL
									var args = {
										route_path: request.route.path,
										host: Proxkey.Responses[request.route.path].response[tmp_current_mode].host,
										path: Proxkey.Responses[request.route.path].response[tmp_current_mode].path,
										port: Proxkey.Responses[request.route.path].response[tmp_current_mode].port,
										method: Proxkey.Responses[request.route.path].response[tmp_current_mode].method ? Proxkey.Responses[request.route.path].response[tmp_current_mode].method : Proxkey.Responses[request.route.path].method
									};
									...
									return reply(Proxkey.Responses[request.route.path].response[tmp_current_mode].data)
					                	.type(Proxkey.Responses[request.route.path].response[tmp_current_mode].type)
					                	.code(Proxkey.Responses[request.route.path].response[tmp_current_mode].code);
								}
							}
						});
					}else {
						console.log('return that shit...');
			...
			//Return defualt response - Success
			return reply(Proxkey.Responses[request.route.path].response.success.data)
					.type(Proxkey.Responses[request.route.path].response.success.type)
		            .code(Proxkey.Responses[request.route.path].response.success.code);
		}
